...
**And a revset is a commit id, never a change id.** The exact opposite answer, to a question that looks like the same one, so the two are stated together. A change id is the right *key* because it survives rewrites; it is the wrong *argument to jj* because it does not always resolve. A **divergent** change — one change id over several visible commits, which is what rewriting an already-obsolete commit produces — makes jj refuse every command that takes it: `Change ID … is divergent`. That shipped, and it reached `jj diff`, `file show`, `evolog`, `resolve --list`, `edit`, `describe`, `rebase`, `duplicate`, `abandon`, `split`, `squash` and `bookmark set`, because the frontend spelled a revision `change.changeId` at every one of those call sites. A divergent change was one you could select and then do nothing with, including look at.

`revisionOf(change)` in `app.ts` is the one place that answer lives, and `App.revsetFor` is it plus the working copy's `null` (which means "diff the live filesystem", not a revision). A commit id resolves to one commit by construction, and it is the better answer anyway: the graph row that was clicked *is* one commit. `interdiff_since_reviewed` is the one command that legitimately takes both, and it names them apart (`change_id` the key, `to_commit` the revision) — it used to derive the second from the first with `repo.log`, which is precisely the pattern that broke. `a_divergent_change_resolves_by_commit_id_and_not_by_change_id` builds a real divergent change and pins it.
